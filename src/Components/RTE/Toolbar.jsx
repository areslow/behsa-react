...
const Toolbar = ({ editorState, setEditorState }) => {
  ...
  return (
    <div className="toolbar-grid py-2 border">
      {tools.map((item, idx) => (
        <button className="m-1"
          style={{
            border: isActive(item.style, item.method)? '1px solid rgba(0, 0, 0, 1)':' 1px solid rgba(0, 0, 0, 0.3)',
      ...
      <button className="m-1 border" style={{width:'40px'}}><FontAwesomeIcon icon={faFileImage} transform="grow-2" /></button>
    </div>
  );
};
